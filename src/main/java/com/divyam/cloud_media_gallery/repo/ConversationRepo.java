...
public interface ConversationRepo extends JpaRepository<Conversation, Long> {
    ...
    @Query(value = "select c.* from conversation as c inner join conversation_members as m on c.id = m.conversation_id inner join user as u on u.id = m.members_id where c.id in (select c.id from conversation as c inner join conversation_members as m on c.id = m.conversation_id where m.members_id = ?1 and c.member_count > ?3 group by c.id having count(*) >= 1) and (c.name regexp ?2 or u.username regexp ?2 or u.first_name regexp ?2) group by c.id having count(*) >= 1 order by c.updated_at desc", countQuery = "select count(c.id) from conversation as c inner join conversation_members as m on c.id = m.conversation_id inner join user as u on u.id = m.members_id where c.id in (select c.id from conversation as c inner join conversation_members as m on c.id = m.conversation_id where m.members_id = ?1 and c.member_count > ?3 group by c.id having count(*) >= 1) and (c.name regexp ?2 or u.username regexp ?2 or u.first_name regexp ?2) group by c.id having count(*) >= 1 order by c.updated_at desc",nativeQuery = true)
    Page<Conversation> findConversationsByMember(long userId, String search, int memberCount,Pageable pageable);
    ...
    @Query(value = "select c.* from conversation as c inner join conversation_members as m on c.id = m.conversation_id inner join user as u on u.id = m.members_id where c.id in (select c.id from conversation as c inner join conversation_members as m on c.id = m.conversation_id where m.members_id = ?1 and c.member_count > ?4 and c.updated_at > ?3 group by c.id having count(*) >= 1) and (c.name like ?2% or u.username like ?2% or u.first_name like ?2%) group by c.id having count(*) >= 1 order by c.updated_at desc", nativeQuery = true)
    List<Conversation> findRecentConversationByMember(long userId, String search, Instant date, int memberCount);
